...
goodAttainment = (
    ...
    " their subjects and is in a great place to move into Year 2. "
)

okAttainment = (
    "{0} {1} has made reasonable progress this academic year, {2} has made the most progress in Science. ",
    "{0} has done well this year, {2} should focus on their phonics where more improvement is needed. ",
...
goodBehaviour = (
    "{0} is very well behaved and sets a fantastic example to their peers. ",
...
if attainment == 1:
    ...
    report += badAttainment[x].format(firstName, secondName, writtenGender)
elif attainment == 2:
    x = random.randint(0, len(okAttainment) - 1)
    report += okAttainment[x].format(firstName, secondName, writtenGender)
else:
    x = random.randint(0, len(goodAttainment) - 1)
    report += goodAttainment[x].format(firstName, secondName, writtenGender)

#report += "\n"

if behaviour == 1:
    x = random.randint(0, len(badBehaviour) - 1)
    report += badBehaviour[x].format(firstName, secondName, writtenGender)
elif behaviour == 2:
    x = random.randint(0, len(okBehaviour) - 1)
    report += okBehaviour[x].format(firstName, secondName, writtenGender)
else:
    x = random.randint(0, len(goodBehaviour) - 1)
    report += goodBehaviour[x].format(firstName, secondName, writtenGender)

#report += "\n"

if (attainment + behaviour) > 4:
    x = random.randint(0, len(praise) - 1)
    report += praise[x].format(firstName, secondName, writtenGender)
else:
    x = random.randint(0, len(decent) - 1)
    report += decent[x].format(firstName, secondName, writtenGender)
